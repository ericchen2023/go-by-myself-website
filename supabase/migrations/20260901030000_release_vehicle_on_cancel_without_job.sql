-- Release the vehicle when a cancel completes with no route job attached.
--
-- 20260901020000 finalised the delivery when the vehicle confirmed a cancel,
-- but only when the cancel carried a route job. A cancel raised after the
-- vehicle has already arrived has none, so that case stayed stuck: the command
-- completed, the delivery stayed in cancel_requested, the reservation stayed
-- active, and the next dispatch failed with VEHICLE_UNAVAILABLE.
--
-- The delivery now comes from the command when the job is absent.
...
begin
  ...
  if command_row.type = 'DISPATCH' and job_row.id is not null then
  ...
  elsif command_row.type = 'CANCEL' and p_event = 'completed' then
    -- A cancel raised after the vehicle has already arrived carries no route job:
    -- execute_delivery_intent looks for one in queued, running or
    -- safe_stop_requested, and arrival left it completed. Requiring a job here
    -- left exactly that case stuck — command completed, delivery still in
    -- cancel_requested, reservation still active. Same lock, other door.
    ...
    if coalesce(job_row.delivery_id, command_row.delivery_id) is null then
      if job_row.id is not null then perform private.release_route_job_reservation(job_row.id); end if;
    else
      ...
      select * into delivery_row from public.deliveries
      where id = coalesce(job_row.delivery_id, command_row.delivery_id) for update;
