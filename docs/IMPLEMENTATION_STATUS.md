...
| Hosted staging control plane | HOSTED + MIGRATIONS PENDING | Supabase `go-by-myself-staging`（project ref `aiuajbflpwdzkaeeocab`，Tokyo）目前套用14個immutable migrations；repository另有第15個route graph v5與第16個public Google OAuth migration待發布。Hosted仍為v4 route active、4個visible stops、0個approved physical legs |
...
| Auth boundary | CODE READY / PROVIDER GATED | 前端已移除`hd`提示與email登入fallback，任何Google帳號首次登入可建立專題帳號；trusted RPC要求Google identity與`email_verified=true`。Google client、Supabase provider、hosted migration與live E2E尚未完成，CTA仍fail closed |
...
| Tests | IMPLEMENTED BASELINE | 54 Vitest、25 Playwright/axe（另 2 個 hosted-only與1個跨 project skip）、10 Python unittest、5 Deno runtime tests、65 hosted pgTAP；auth migration發布後為67個（RLS 27＋integration 40），另有 contract/build/boundary/bundle、protected staging health與production shell E2E |
...
- Public Google OAuth的程式與migration已完成，但Google OAuth client、Supabase provider、hosted migration、Vercel flag及真實一般Google帳號E2E尚未完成；Google provider與CTA維持disabled。
...
| Production-shaped staging | HOSTED CONTROL-PLANE READY / FULL E2E PENDING；既有DB、RLS、Edge、Vercel與robot HTTP已驗證，public Google OAuth migration/provider、Realtime與multi-context E2E未完成 |
